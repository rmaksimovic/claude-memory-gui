...
// Run `claude -p <prompt>`. Returns a Promise<string> with the output.
...
async function summarizeFile(filePath) {
  ...
  const prompt =
    ...
    'Return 3 to 5 concise bullet points (using "•"). ' +
    'Describe what type of information is stored, the key facts or decisions, and its purpose. ' +
    'Be brief and direct.\n\nFILE:\n' + content;
  return runClaudePrompt(prompt);
}
...
module.exports = { parseConversationMeta, listConversations, parseFullConversation, forkConversation, summarizeConversation, summarizeFile, computeProjectCost };
